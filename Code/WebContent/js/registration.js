...
function modifyUser(){	
	...
	$("#companyNumber").prop("readonly", true);

}

function modifyProduct(){

	var tabArray = JSON.parse(sessionStorage.getItem( "tableRow" ));
	
	$.each(tabArray, function( index, value ) {
		
		if( value[0] == modifyProductId ){
			$("#productName").val(value[1]);
			$("#modelNumber").val(value[2]);
			$("#os").val(value[3]);
			$("#ram").val(value[4]);
			$("#hardDisk").val(value[5]);
			$("#processor").val(value[6]);
			$("#screenSize").val(value[7]);
			$("#available").val(value[8]);
			$("#price").val(value[9]);
			return false;
		}
	})
