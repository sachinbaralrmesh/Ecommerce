...
function ajaxCallForFavouriteList(){	

	$("#tableHeader").html("Favourite List");
...
function callFunction(title){
	...
	$.each(array, function( index, value ) {
		...
		if( value[0] == title ){
			...
			$.each(cartArray, function(cartIndex, cartValue){
				
				if( value[0] == cartValue[0] ){
					alreadyInCart = true;
				}
			})
			...
			if( !alreadyInCart ){
				...
				value[10] = "<button title='" + value[0] + "' class='submit_button top' onclick='deleteFromCart(this.title);'>Delete from cart</button>";
				value[8] = selectString;
				cartArray.push(value);
				alert("Product added to cart");
			}else{
				alert("Product exists in cart already");
			}
...
function modifyProduct(title){
	...
	sessionStorage.setItem("tableRow", JSON.stringify(array));
