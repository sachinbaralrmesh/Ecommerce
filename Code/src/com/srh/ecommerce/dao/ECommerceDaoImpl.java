...
import com.srh.ecommerce.view.OrderSpec;
...
public class ECommerceDaoImpl{
	...
	public List<Product> getProductList(
		...
		if( !companyNumber.equalsIgnoreCase("") ){
			...
			if( !searchParamString.equalsIgnoreCase("") ) sql = sql + " AND UPPER(PRODUCTNAME) LIKE :searchParam";	

			queryMap = new HashMap<String, String>();

			queryMap.put("status", "ACTIVE");
			if( !searchParamString.equalsIgnoreCase("") ) queryMap.put("searchParam", "%" + searchParamString.toUpperCase() + "%" );
	...
	private String getMaxOrderNumber(){
		...
		Long maxOrderNumber = jdbcTemplate.queryForLong("select max(TO_NUMBER(SUBSTRB(ORDER_NUMBER,4,LENGTH(ORDER_NUMBER)-3))) from orders");

		String newOrderNumber="OR "+(maxOrderNumber+1);
	...
	//	public long insertOrder(List<String> productIds,String sessionUserName){
	//		
	//		NamedParameterJdbcTemplate namedJdbcTemplate= getNamedJdbcTemplate();
	//		JdbcTemplate jdbcTemplate=getJdbcTemplate();
	//		
	//		String sql="select max(TO_NUMBER(SUBSTRB(ORDER_NUMBER,4,LENGTH(ORDER_NUMBER)-3))) from orders";
	//		long maxOrderNumber=jdbcTemplate.queryForLong(sql);
	//		String newOrderNumber="OR "+(maxOrderNumber+1);
	//		sql="select max(orderid) from orders";
	//		long maxOrderId=jdbcTemplate.queryForLong(sql);
	//		
	//		
	...
	//			
	//		sql="insert into  orders VALUES(:maxOrderId,:newOrderNumber,:productId,:sessionUserName,TO_CHAR(CURRENT_DATE),TO_CHAR(CURRENT_DATE+3),null,'SHIPPING')";
	//		Map<String, String> sqlMap= new HashMap<String, String>();
	//		sqlMap.put("maxOrderId",String.valueOf(maxOrderId));
	//		sqlMap.put("newOrderNumber", newOrderNumber);
	//		sqlMap.put("productId", productId);
	//		sqlMap.put("sessionUserName", sessionUserName);
	//		
	//		namedJdbcTemplate.update(sql, sqlMap);
	//			}
	...
	public String insertOrders(OrderSpec spec, String userName) {
		...
		String newOrderNumber= getMaxOrderNumber();

		String sql = "INSERT INTO ORDERS VALUES(:orderId, :orderNumber, :productId, :userName, TO_CHAR(CURRENT_DATE),"
				+ " TO_CHAR(CURRENT_DATE), :shippingAddress, :noOfItems, :orderStatus, :url)";
		...
		for( String l : spec.getProducts() ){
			...
			queryMap.put("orderNumber", newOrderNumber);
	...
	public String getCompanyNumber(String companyName) {
		...
		String sql = "SELECT (SELECT COMPANYNUMBER FROM COMPANY WHERE COMPANYNAME = :companyName) AS COMPANYNUMBER FROM DUAL";
		...
		return companyNumber == null ? "N/A" : companyNumber;
	}
	...
	public void updateUrlForTheOrder(String url, String orderNumber) {

		NamedParameterJdbcTemplate namedJdbcTemplate = getNamedJdbcTemplate();

		String sql = "UPDATE ORDERS SET URL = :url WHERE ORDER_NUMBER = :orderNumber";
