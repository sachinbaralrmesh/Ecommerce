...
import com.srh.ecommerce.view.AdminSearch;
...
public class ECommerceServiceImpl {
	...
	public String orderConfirmation(OrderSpec spec, String userName) throws COSVisitorException, IOException {
		...
		daoImpl.updateUrlForTheOrder(url, orderNumber);
	...
	private List<Object> getObjectArrayFromList(List<Orders> orderList, String userType){
		...
		if( orderList != null ){
			...
			for(Orders mapper : orderList){
				...
				StringBuilder downloadUrl = new StringBuilder("<input type='button' class='submit_button top' name='");
				downloadUrl.append(mapper.getUrl());
				downloadUrl.append("' onclick='downloadConfirmation(this.name);' value='Download'>");
				...
				arrayList.add(downloadUrl.toString());
